...
func main() {
	...
	actionConditions, err := deriveActionConditions(rules)
	if err != nil {
		fmt.Fprintf(os.Stderr, "derive action conditions: %v\n", err)
		os.Exit(1)
	}
	...
	meta, err := generateActionMetaSource(actionArgSchemas, actionRequiredArgs, actionScopes, actionConditions)
	if err != nil {
...
func generateActionMetaSource(actionArgSchemas map[string]map[string]string, actionRequiredArgs map[string][]string, actionScopes map[string][]string, actionConditions map[string]string) ([]byte, error) {
	var b bytes.Buffer
	b.WriteString("// Code generated by cmd/genactions; DO NOT EDIT.\n")
	...
	b.WriteString("var builtInActionConditions = map[string]string{\n")
	conditionActionNames := make([]string, 0, len(actionConditions))
	for action := range actionConditions {
		conditionActionNames = append(conditionActionNames, action)
	}
	sort.Strings(conditionActionNames)
	for _, action := range conditionActionNames {
		b.WriteString(fmt.Sprintf("\t%q: %q,\n", action, actionConditions[action]))
	}
	b.WriteString("}\n\n")
	...
	b.WriteString("func ActionWhen(action string) string {\n")
	b.WriteString("\treturn builtInActionConditions[strings.TrimSpace(action)]\n")
	b.WriteString("}\n\n")
...
func parseBindDirectives(doc *ast.CommentGroup, intentType string) []parsedRule {
	...
	for _, c := range doc.List {
		...
		parts := splitDirectiveParts(rest)
		...
		for _, part := range parts {
			...
			case "when":
				if strings.HasPrefix(v, `"`) {
					...
					if err != nil {
						errs = append(errs, fmt.Errorf("invalid when condition in directive %q: %w", line, err))
						continue
					}
					v = value
				}
...
func splitDirectiveParts(input string) []string {
	var parts []string
	start := -1
	quoted := false
	escaped := false
...
func deriveActionConditions(rules []bindRule) (map[string]string, error) {
	...
	for _, rule := range rules {
		...
		if _, err := actionmeta.ParseCondition(when); err != nil {
			return nil, fmt.Errorf("action %q: %w", canonicalActionID(rule.Scope, rule.Action), err)
		}
		action := canonicalActionID(rule.Scope, rule.Action)
		if previous, ok := conditions[action]; ok && previous != when {
			return nil, fmt.Errorf("action %q has conflicting when conditions %q and %q", action, previous, when)
		}
		conditions[action] = when
	}
